...
//            Notifications and email content
    ...
    public function pendingVehicles(){
        $user_type = auth()->user()->user_type;
        if ($user_type == 'admin' or $user_type == "Admin"){
        $data['vehicles'] = $this->vehicle->where('status','pending')->
        orderBy('id','desc')->get();
        }else{
    ...
    public function approvedVehicles(){
        $user_type = auth()->user()->user_type;
        if ($user_type == 'admin' or $user_type == "Admin"){
        $data['vehicles'] = $this->vehicle->where('status', 'approved')->
        orderBy('id', 'desc')->get();
        }else{
    ...
    public function disapprovedVehicles(){
        $user_type = auth()->user()->user_type;
        if ($user_type == 'admin' or $user_type == "Admin"){
        $data['vehicles'] = $this->vehicle->where('status','disapproved')->
        orderBy('id','desc')->get();
        }else{
            $data['vehicles'] = $this->vehicle->where('status','disapproved')->
    ...
    public function blockedVehicles(){
        $user_type = auth()->user()->user_type;
        if ($user_type == 'admin' or $user_type == "Admin") {
            $data['vehicles'] = $this->vehicle->where('status', 'blocked')->
            orderBy('id', 'desc')->get();
        }else{
